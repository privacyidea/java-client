...
public class TestU2F
{
    ...
    public void test()
    {
        ...
        mockServer.when(HttpRequest.request().withPath(PIConstants.ENDPOINT_VALIDATE_CHECK).withMethod("POST").withBody(
                          "user=Test&transaction_id=16786665691788289392&pass=&clientdata=eyJjaGFsbGVuZ2UiOiJpY2UBc3NlcnRpb24ifQ&signaturedata=AQAAAxAwRQIgZwEObruoCRRo738F9up1tdV2M0H1MdP5pkO5Eg"))
        ...
        Optional<Challenge> opt = response.multiChallenge().stream()
                                          .filter(challenge -> TOKEN_TYPE_U2F.equals(challenge.getType())).findFirst();
